...
public class MoveWithMouse : MonoBehaviour
{
    ...
    private void GetInput()
    {
        if (Input.GetMouseButton(0))
        {
            ...
            Vector2 hitPoint = Physics2D.Raycast(transform.position, _mousePosition - transform.position, _layerMask).point;
            Debug.Log(hitPoint);
            //Debug.DrawRay(transform.position, (new Vector3( hitPoint.x, hitPoint.y, transform.position.z) - transform.position));
